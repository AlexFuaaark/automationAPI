...
describe('USER SIGNUP', () => {
    describe('POSITIVE TESTING', () => {
        it('Create a new user', async () => {
            const res = await request.post("/users/signup")
                .send({
                    "name": "Mike",
                    "email": "mike9999@gmail.com",
                    "password": "pass1234",
                    "passwordConfirm": "pass1234"

                }).expect(201)
            expect(res.body.data.user.name).toBe('Mike')
            expect(res.body.data.user.email).toBe('mike9999@gmail.com')
    ...
    describe('NEGATIVE TESTING', () => {
        it.only('should not create a new user with an existing email', async () => {
            ...
            expect(res.body.message).toBe('E11000 duplicate key error collection: test.users index: email_1 dup key: { email: \"mike9999@gmail.com\" }');
        })
    })
});
